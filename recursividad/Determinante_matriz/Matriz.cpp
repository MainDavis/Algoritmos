...
double Matriz::calcularDeterminante(){
	assertdomjudge(this->n_filas == this->n_columnas && this->n_filas>0 && this->n_columnas>0);
	double det=0;

	if(this->n_filas == 2) return ( (matriz[0][0]*matriz[1][1]) - (matriz[0][1]*matriz[1][0]) );
	else{

		Matriz *arrMatrices;
		arrMatrices = new Matriz[this->n_filas];
		
		for(int i=0; i<n_filas; i++){
			arrMatrices[i] = Matriz(this->n_filas-1, this->n_filas-1);
			for(int j=1; j<n_filas; j++){
				...
				for(int k=0; k<n_filas; k++){
					...
					arrMatrices[i].matriz[j-1][k-temp] = this->matriz[j][k];
					if(i==k+1){
						k++;
						temp++;
					}
			...
			det+= pow(-1,i)*this->matriz[0][i]*arrMatrices[i].calcularDeterminante();
		}
		return det;
	}
}
